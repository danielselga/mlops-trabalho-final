...
def call_modelo02(request = request):
    ...
    json_ = request.json
    campos2 = pd.DataFrame(json_)

    if campos2.shape[0] == 0:
        return "Dados de chamada da API estão incorretos.", 400

    print("Predizendo com modelo 2 para {0} registros".format(campos2.shape[0]))

    for col in modelo02.independentcols:
        if col not in campos2.columns:
            campos2[col] = 0
    x = campos2[modelo02.independentcols]

    prediction2 = modelo02.predict(x)

    ret = json.dumps({'prediction': list(prediction2)}, cls=NpEncoder)
